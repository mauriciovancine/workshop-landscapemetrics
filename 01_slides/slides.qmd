...
1. **Geoprocessamento**: entende padrões espaciais a partir de dados espaciais para gerar informações espaciais, com quatro componentes: cartografia, GPS, SR e SIG
...
1. **GPS**: indica sua localização no planeta através de pontos ou linhas
1. **Sensoriamento Remoto**: usa sistemas sensores acoplados a satélites para imagear a superfície do planeta de forma remota 
1. **SIG**: conjunto de ferramentas que tratam os dados geoespaciais para gerar informações
1. **Vetor**: pontos, linhas e polígonos em que há associado uma tabela de dados
1. **Raster**: matriz de valores contínuos ou categóricos
1. **Conversão**: de dados vetoriais para dados rasters e vice-versa
...
Caminho de uma pasta/diretório onde o R irá importar e exportar os dados
...
```{r fig.align='center'}
#| echo = TRUE
plot(rc$geom, axes = TRUE, graticule = TRUE)
```
...
```{r fig.align='center'}
#| echo = TRUE
plot(rc_utm$geom, axes = TRUE, graticule = TRUE)
```
...
# reprojetar vetor
ra_utm <- raster::projectRaster(ra, crs = "+init=epsg:31983", method = "bilinear", res = 90)
ra_utm 
```
